...
function printDeals(deals, inputs, bookKey, colIdx) {
  ...
  const tableRows = deals.map(({ row, price, book, otherBook, amtFin, payment, equityPct, tax }, rank) => {
    ...
    const wholesale = bookKey === 'WHOLESALE / TRADE-IN' ? book : otherBook;
    const retail    = bookKey === 'RETAIL / MSRP' ? book : otherBook;
    return `<tr>
      ...
      <td style="text-align:right;font-family:monospace${bookKey === 'WHOLESALE / TRADE-IN' ? ';font-weight:700;color:#111' : ''}">${fmt$(wholesale)}</td>
      <td style="text-align:right;font-family:monospace${bookKey === 'RETAIL / MSRP' ? ';font-weight:700;color:#111' : ''}">${fmt$(retail)}</td>
      ...
      <main className={styles.resultsMain}>
        ...
        {displayDeals.length === 0 ? (
          ...
          <div className={styles.dealCards}>
            {displayDeals.map(({ row, i, price, book, otherBook, amtFin, payment, equityPct, tax }, rank) => {
              ...
              return (
                <div key={i} className={`${styles.dealCard} ${isNeg ? styles.dealCardNeg : styles.dealCardPos}`}>
                  ...
                  <div className={styles.dealFinancials}>
                    ...
                    <div className={styles.dealFin}>
                      <span className={styles.dealFinLabel}>Wholesale{bookKey === 'WHOLESALE / TRADE-IN' && <span className={styles.dealFinActiveDot} />}</span>
                      <span className={styles.dealFinVal}>{fmt$(bookKey === 'WHOLESALE / TRADE-IN' ? book : otherBook)}</span>
                    </div>
                    <div className={styles.dealFin}>
                      <span className={styles.dealFinLabel}>Retail{bookKey === 'RETAIL / MSRP' && <span className={styles.dealFinActiveDot} />}</span>
                      <span className={styles.dealFinVal}>{fmt$(bookKey === 'RETAIL / MSRP' ? book : otherBook)}</span>
                    </div>
                    <div className={styles.dealFin}>
